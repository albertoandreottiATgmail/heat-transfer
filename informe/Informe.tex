...
The input to the map functions is taken from the HDFS filesystem. Then
...
shuffles, a sort, and a set of reduce tasks, as it was depicted in \ref{fig:map_red}. \\
...
length, linear size, and other data related to the Matrix.
...
The next option we explore, distributes data with a coarser granularity pattern.
...
results are in Figure \ref{fig:perf}. There is a marked difference between both 
implementation, being the coarse grained faster by a factor ranging from $4$
using three node to $7$ when only one node is used.\\
As it was explained in the previous sections, this is primarily due to the overload
to spawn the map and reduce functions that takes place in the fine grained version 
compared to the coarse grained one; it is $L$x$L$ in the first one, and $L$ in the 
second. In general, fewer map and reduce functions will result in better performance.\\
Furthermore, the amount of data that is moved is different in both versions. For every element of the 
matrix, \emph{four} outputs are produced in the fine grained version. On the contrary, for
the coarse grained version, for every row, the map produces only \emph{three} output rows.
Consequently, not only few data has to be moved, but also fewer keys have to be sorted
...
and fault tolerance.\\
Furthermore, the execution model of Hadoop involves many phases that are form a complex
graph of activities. Detaching the strictly computational part of the execution in order
to provide an acceptable measure for comparison is challenging. 

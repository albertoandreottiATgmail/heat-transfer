...
much of the material found in this paper was omitted, so we encourage the interested reader to take
...
public void reduce(IntWritable key, Iterable<KeyArrayValue> values, 
  ...
  if(kav.getKey()==key.get()){
	...
	for(int i=1; i<(MatrixData.Width()-1);i++){
		tmp = ((FloatWritable[])kav.toArray())[i-1].get() + 
        		((FloatWritable[])kav.toArray())[i+1].get();
...
during much of the configuration work that had to be done in order to 
